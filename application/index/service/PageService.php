...
class PageService extends Service
{
    ...
    public function head($plugins)
    {
        ...
        foreach ($plugins as $pluginPath) {
            ...
            foreach ($pluginInfo as $k => $plugin) {
                ...
                if (isset($plugin['js'])) {
                    $jsStr .= '<script src="/node_modules/'.$plugin['js'].'"></script>'."\r\n";
                }
                if (isset($plugin['css'])) {
                    $cssStr .= '<link rel="stylesheet" href="/node_modules/'.$plugin['css'].'">'."\r\n";
                }
                $installPlugins[] = $k;
            }
        }
    ...
    public function js($pageInfo, $jsPaths)
    {
        ...
        if (!is_dir(ROOT_PATH.'public/js/'.$pageInfo['name'])) {
            mkdir(ROOT_PATH.'public/js/'.$pageInfo['name']);
        }
    ...
    public function css($pageInfo, $cssPaths)
    {
        ...
        if (!is_dir(ROOT_PATH.'public/css/'.$pageInfo['name'])) {
            mkdir(ROOT_PATH.'public/css/'.$pageInfo['name']);
        }
